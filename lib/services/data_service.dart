...
class DataService {
  ...
  Future<List<IndustrialDataModel>> loadData({int from = 0, int to = 19}) async {
    try {
      ...
      print('Error loading data in DataService: $e');
      print(stackTrace);
      rethrow;
    }
  }

  Future<List<IndustrialDataModel>> loadAllData() async {
    try {
      List<IndustrialDataModel> allData = [];
      int from = 0;
      ...
      while (hasMore) {
        ...
        final response = await _supabase
            .from('industrial_data')
            .select()
            .order('fecha', ascending: false) // Order ensures consistency while paging
            .range(from, to)
            .timeout(const Duration(seconds: 30));

        final List<IndustrialDataModel> chunk = (response as List)
            .map((json) => IndustrialDataModel.fromJson(json))
            .toList();
      ...
      print('Error loading all data in DataService: $e');
      print(stackTrace);
      rethrow; // Or return empty list, but rethrow lets provider know
    }
  }
  ...
  Future<bool> updateData(IndustrialDataModel data) async {
    try {
      ...
      json.remove('id'); // Don't update the ID itself
      await _supabase
          .from('industrial_data')
          .update(json)
          .eq('id', data.id!);
